...
// (pin_key_event / clear_pin_event / submit_pin_event) and named widgets
// ("pin_label" / "message_label") that main.cpp already looks for, so the
// firmware builds and the PIN flow works. Replace with the real
// SquareLine export whenever it's available.
...
lv_obj_t *login_gen_create(void)
{
    ...
    lv_obj_set_name(message_label, "message_label");
    ...
    lv_obj_set_name(pin_label, "pin_label");
